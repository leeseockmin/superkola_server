...
using Common.Database.Entity;
using Common.Network.PacketHandling;
using Common.Network.ServerManaging;
using Common.Network.Session;
using Common.Redis;
using Common.Setting;
using Common.Setting.FileConfig;
using Common.SQS;
using Common.Util;
using FlatMessages.Datas;
using GameCommon.Entity.Game;
using GameCommon.Entity.Game.Logic;
using GameCommon.Entity.World;
using GameCommon.Ranking;
using GameCommon.Redis;
using GameCommon.Resources.RefinedResources;
using GameCommon.Statistics;
using GameCommon.Utils;
using GameServer.Contents.Common;
using GameServer.Contents.Global;
...
using static GameCommon.Statistics.Archive;
using System.Collections.Generic;
using GameServer.Contents.Model;
using static GameServer.Contents.Model.LabyrinthNodeModel;
using static GameServer.Contents.Common.LabyrinthCommon;
...
namespace GameServer.PacketHandling.UserHandlers;

[PacketAttribute<EUserState>(ESessionType.User, EUserState.Idle)]
public static class IdleUserHandler
{
    public static bool RecvMessage<TTable>(out TTable req, UserSession session, FlatMessage message) where TTable : struct, IFlatbufferObject
    {
        req = message.MessageAs<TTable>();
    ...
    public static async Task Database(UserSession session, EDatabaseType type, Func<BaseDatabaseConnection, Task> action, System.Action<Exception>? exceptionAction = null)
    {
        using var connectionWrapper = await DatabaseManager.Instance.GetDisposableConnectionAsync(type);
        ...
        catch (Exception e)
        {
            ...
            {
                SessionManager.Kick(session, EResultType.LogoutRollback);
                PacketSender.Send_SC_ServerError(session, e);
            }
        }
    }
